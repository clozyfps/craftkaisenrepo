			...
			_level.sendParticles(ParticleTypes.BUBBLE, x, y, z, 10, 8, 4, 8, 0);
		{
			...
			for (Entity entityiterator : _entfound) {
				if (!(entityiterator == immediatesourceentity || entityiterator == entity)) {
					...
					entityiterator.hurt(
							new DamageSource(world.registryAccess().registryOrThrow(Registries.DAMAGE_TYPE).getHolderOrThrow(DamageTypes.DROWN), immediatesourceentity, (entity instanceof TamableAnimal _tamEnt ? (Entity) _tamEnt.getOwner() : null)),
							3);
