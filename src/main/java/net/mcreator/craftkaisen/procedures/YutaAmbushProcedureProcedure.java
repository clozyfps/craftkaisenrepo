...
public class YutaAmbushProcedureProcedure {
	public static void execute(LevelAccessor world, double y, double z, Entity entity) {
		...
		if (world.getBiome(BlockPos.containing(entity.getX(), entity.getY(), entity.getZ())).is(TagKey.create(Registries.BIOME, new ResourceLocation("craft_kaisen:yuta_forest_eligible")))) {
			...
			if (world.getBlockState(BlockPos.containing(entity.getPersistentData().getDouble("yutax"), entity.getY(), entity.getZ())).canOcclude()) {
			...
			} else if (!world.getBlockState(BlockPos.containing(entity.getPersistentData().getDouble("yutax"), entity.getY(), entity.getZ())).canOcclude()) {
