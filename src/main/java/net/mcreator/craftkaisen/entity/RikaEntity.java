...
public class RikaEntity extends TamableAnimal implements GeoEntity {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(RikaEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(RikaEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(RikaEntity.class, EntityDataSerializers.STRING);
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "rikatexture");
	}
	...
	public boolean isFood(ItemStack stack) {
		return List.of(CraftKaisenModItems.TEST.get()).contains(stack.getItem());
	}
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.animationprocedure.equals("empty")) {
			if ((event.isMoving() || !(event.getLimbSwingAmount() > -0.15F && event.getLimbSwingAmount() < 0.15F))
				...
				return event.setAndContinue(RawAnimation.begin().thenLoop("rikamove"));
			}
			return event.setAndContinue(RawAnimation.begin().thenLoop("rikaidle"));
		}
	...
	private PlayState attackingPredicate(AnimationState event) {
		...
		if (this.swinging && event.getController().getAnimationState() == AnimationController.State.STOPPED) {
			...
			return event.setAndContinue(RawAnimation.begin().thenPlay("rikaslash"));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 20) {
			this.remove(RikaEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
